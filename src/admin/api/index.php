...
try {
    ...
    function getStudents($db) {
        try {
            ...
            if (!in_array($sort, $allowedSortFields, true)) $sort = 'name';
    ...
    function createStudent($db, $data) {
        try {
            if (empty($data['student_id']) || empty($data['name']) || empty($data['email']) || empty($data['password'])) {
                sendResponse(['success' => false, 'message' => 'student_id, name, email, and password are required'], 400);
            }
            ...
            if ($existing) {
                sendResponse(['success' => false, 'message' => 'student_id or email already exists'], 409);
            }
            ...
            sendResponse(['success' => true, 'message' => 'Student created successfully', 'data' => $student], 201);
        } catch (PDOException $e) {
            sendResponse(['success' => false, 'message' => 'Database error: ' . $e->getMessage()], 500);
        } catch (Exception $e) {
            sendResponse(['success' => false, 'message' => 'Error: ' . $e->getMessage()], 500);
    ...
    // يمكن إضافة باقي الدوال updateStudent, deleteStudent, changePassword بنفس طريقة try-catch
